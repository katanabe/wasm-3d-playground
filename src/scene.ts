...
export async function createScene(canvas: HTMLCanvasElement) {
  ...
  // Build shapes (including async glTF)
  const shapes: Record<ObjectShape, ShapeDef> = {
    conifer: {
  ...
  const coniferColors = [0x2a5540, 0x305a45, 0x254a3a, 0x356050, 0x204535];
  ...
  function setInstances(positions: Float32Array) {
    ...
    const useColorVariation = currentShape === "conifer";
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (useColorVariation) {
        color.setHex(coniferColors[placed % coniferColors.length]);
